...
/*
請文字解釋為什麼
console.log(k * l > 21); 結果為true，
運算子*優先度高於與運算子>，故先執行 k * l，
又string型別碰到*運算子時，會隱性嘗試轉換為number型別計算，string型別'8'可成功轉換，故實際判斷為3*8是否大於21。
*/
console.log(k * l > 21); 
...
const gymCoachs = [
  {
    ...
    introduce:'擁有 5 年教學經驗，專精於提升學員的肌力與減脂，適合希望快速達成體能目標的學員',
    isReceiveStudent:true
  },
  {
    name:'李教練',
    skill:'瑜伽、體態雕塑',
    ...
    introduce:'一位瑜伽大師，擁有 10 年教學經驗，擅長幫助學員雕塑完美體態，適合希望改善姿態與柔軟度的學員',
    isReceiveStudent:false
  }
]; // 練習：使用物件變數定義兩位教練的資訊

console.log(gymCoachs);
...
if (performanceData.coaches[0].performance < performanceTarget) {
  performanceData.coaches[0].performance += (performanceTarget - performanceData.coaches[0].performance + 1);
}

// 練習：第二位教練（可將下方程式碼註解移除，完成答題）

if (performanceData.coaches[1].performance < performanceTarget) {
  performanceData.coaches[1].performance += (performanceTarget - performanceData.coaches[1].performance + 1);
}
...
let calorieBurn = 0;
// 練習：計算小明今日消耗的卡路里
calorieBurn += (activities.bicycle.calorieBurnPerMinute * 10);
calorieBurn += (activities.runningMachine.calorieBurnPerMinute * 30);
...
exerciseRecords.forEach( (dayRecord) =>{
  if (dayRecord.duration >= 30 && dayRecord.heartRate >= 130) {
    totalDuration += dayRecord.duration;
    validDays += 1;
  }
});
